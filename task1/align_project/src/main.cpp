...
Matrix<double> parse_kernel(string kernel)
{
	...
	for(uint j=0;j<n;++j)
	{
		string::size_type tpos = kernel.find(',',pos)<kernel.find(';',pos) ? kernel.find(',',pos) : kernel.find(';',pos);
		if(tpos==string::npos)
			tpos=kernel.size();
		string t=kernel.substr(pos,tpos-pos);
		...
		{
			string t1=t.substr(0,tpos), t2=t.substr(tpos+1);
			ret(i,j)=std::stod(t1)/std::stod(t2);
			continue;
		}
	}
    return ret;
...
int main(int argc, char **argv)
{
    try {
        ...
        if (action == "--sobel-x") {
            ...
            dst_image = align(src_image, isPostprocessing, postprocessingType, fraction, isMirror, isInterp, isSubpixel, subScale);
        } else {
